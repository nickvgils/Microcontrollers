...
** main.c
**
** Beschrijving:	Walking light on a 7 segment display, using a lookup table
** Target:			AVR mcu
...
PATTERN_STRUCT pattern[] = {
	{0b00000001, 5000}, {0b00000010, 5000}, {0b00000100, 5000}, {0b00001000, 5000},{0b00010000, 5000},{0b00100000, 4000}
}; // circle pattern, walking light
...
*******************************************************************/
{
	...
	PORTB = 0b11111111;	// PORTB input
		...
		while (1)
		{
			...
			// Write data to PORTB
			DDRB = pattern[index].data; // portB is set to the data field of the // pattern index
			wait(pattern[index].delay); // Wait is set to the delay field of the // pattern index
			
			
			if(pattern[index].delay == 4000) // If the counter has reached an element with a delay of 4000
			{
